...
class BrowserAgent:
    ...
    def __init__(self, session_id: Optional[str] = None, multi_mcp: Any = None):
        ...
        self.form_state = {
            'filled_fields': set(),  # Track successfully filled fields
            'attempted_actions': [],  # Track attempted actions
            'current_form_data': {},  # Store form data for this session
            'submission_attempted': False
        }
        
        # Element stability tracking to handle ID flickering
        self.element_stability = {
            'last_elements_snapshot': None,  # Cache of last elements scan
            'stable_element_map': {},        # Persistent element mapping by attributes
            'element_history': [],           # Track element changes over time
            'last_scan_timestamp': None,     # When last scan occurred
            'scan_count': 0                  # Number of scans performed
        }
