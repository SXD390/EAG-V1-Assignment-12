...
class EnhancedAgentLoop:
    ...
    async def _run_browser_workflow(self, query: str):
        """Run workflow with STANDALONE browser agent - no callbacks to perception/decision"""
        try:
            self.logger.info("🌐 Starting STANDALONE browser workflow")
            
            # Initialize browser agent if needed
            if not self.browser_agent:
                self.browser_agent = await create_browser_agent(self.session_id, self.multi_mcp)
                
            # Execute the COMPLETE browser workflow internally
            result = await self.browser_agent.execute_complete_workflow(query)
            
            # Extract meaningful information from browser results
            meaningful_info = await self._extract_meaningful_browser_info(result, query)
            
            # Store results in context with meaningful information
            if isinstance(result, dict) and result.get("results"):
                self.ctx.add_browser_results(result["results"])
            
            # Update perception output with browser results for proper summarization
            if meaningful_info:
                # Create enhanced perception output that includes browser findings
                enhanced_p_out = {
                    **getattr(self, 'p_out', {}),
                    "browser_results": meaningful_info,
                    "last_tooluse_summary": meaningful_info.get("summary", ""),
                    "original_goal_achieved": meaningful_info.get("goal_achieved", False),
                    "local_goal_achieved": meaningful_info.get("goal_achieved", False),
                    "solution_summary": meaningful_info.get("extracted_content", "")
                }
                self.p_out = enhanced_p_out
            
            # Return final summary without going back to perception/decision
            if result.get("success") and meaningful_info:
                self.status = "success" 
                return await self._summarize()
            else:
                error_msg = result.get('error', 'No meaningful information extracted from browser workflow')
                return f"Browser workflow failed: {error_msg}"
                
        except Exception as e:
            return await self._handle_agent_error("browser", str(e))

    async def _extract_meaningful_browser_info(self, browser_result: Dict[str, Any], original_query: str) -> Optional[Dict[str, Any]]:
        """Extract meaningful information from browser workflow results for summarization"""
        try:
            from agent.model_manager import ModelManager
            model = ModelManager()
            
            # Get the results from browser workflow
            results = browser_result.get("results", [])
            if not results:
                return None
            
            # Find the most meaningful results (usually the last few actions)
            meaningful_results = []
            for result in results[-5:]:  # Look at last 5 actions
            ...
            # Use LLM to extract and format meaningful information
            prompt = f"""Extract meaningful information from browser automation results to answer the user's query.

ORIGINAL QUERY: {original_query}

BROWSER RESULTS:
{json.dumps(meaningful_results, indent=2)}

Extract the key information that answers the user's query. Focus on:
1. What information was successfully extracted or found?
2. What specific content answers the user's question?
3. Was the goal achieved?

Return ONLY a JSON object:
{{
    "goal_achieved": true/false,
    "extracted_content": "The main information that answers the user's query",
...
"""
            ...
            try:
                ...
                self.logger.info(f"📋 Extracted meaningful info: {info.get('summary', '')}")
                return info
                
            except json.JSONDecodeError as e:
                self.logger.error(f"Failed to parse meaningful info extraction: {e}")
                return None
                
        except Exception as e:
            self.logger.error(f"Failed to extract meaningful browser info: {e}")
            return None
    ...
    async def _execute_steps_loop(self):
        ...
        from agent.agent_loop3 import StepExecutionTracker
        from action.execute_step import execute_step_with_mode
        ...
        while tracker.should_continue():
            ...
            if self.p_out.get("original_goal_achieved") or self.p_out.get("route") == "summarize":
                self.status = "success"
                self.final_output = await self._summarize()
                return

            if self.p_out.get("route") != "decision":
        ...
        for node in plan_graph["nodes"]:
            ...
            if step_id in ctx.graph.nodes:
                ...
                if existing.status != "pending":
                    continue
            ctx.add_step(step_id, description=node["description"], step_type="CODE", from_node=from_step_id)
